...
then
	...
	echo Which optimizer should be used \(available option is \"mosek\", currently set to \"$OPTIMIZER\"\)?
	read OPTIMIZER
	if [ "$OPTIMIZER" != 'mosek'];
		then
...
grep -v -e OCTAVE_BIN_PATH -e OCTAVE_MKOCT -e MATLAB_BIN_PATH -e MATLAB_MEX_PATH -e MATLAB_INCLUDE_DIR \
    ...
    -e SAMTOOLS_DIR -e PYTHON_PATH -e SCIPY_PATH -e MOSEKLM_LICENSE_FILE -e $RQUANT_VERSION > bin/rquant_config.sh
