...
%%%%% experiment %%%%%
CFG.organism = 'alyrata';
CFG.exp = {{'lyrata_lisa_run_1', 'lyrata_lisa_run_3'}, {'lyrata_lisa_run_2', 'lyrata_lisa_run_4'}}; % run 1+3: replicate 1, run 2+4: replicate 2
PAR.CFG.read_len = 75;

%%%%% tracks, repeats, genes, genome info %%%%% 
CFG.base_dir = '/fml/ag-raetsch/share/projects/rquant/alyrata';
PAR.CFG.repeats_fn = '';
...
for e = 1:length(CFG.exp),
  for f = 1:length(CFG.exp{e}),
    PAR.track{f} = sprintf('%s/tracks/%s.bam', CFG.base_dir, CFG.exp{e}{f});
  end
  %%%%% result directory %%%%%
  date_exp = datestr(now,'yyyy-mm-dd');
  PAR.output_dir = sprintf('%s/rquant/%s_%s', CFG.base_dir, CFG.exp{e}{1}, date_exp);
