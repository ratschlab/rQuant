function write_rquant_gff3(CFG, genes, gff_fname, mapped_reads) 
% WRITE_RQUANT_GFF3   Writes rQuant results in gff3 format.
%
% write_rquant_gff3(CFG, genes, gff_fname, mapped_reads)
%
%   -- input --
...
if exist(gff_fname, 'file')
  if CFG.VERBOSE>1, fprintf(1, 'replacing file %s...\n', gff_fname); end
  [fd msg] = fopen(gff_fname, 'w+');
  if CFG.VERBOSE>1, disp(msg); end
else
  if CFG.VERBOSE>1, fprintf(1, 'creating file %s...\n', gff_fname); end
  [fd msg] = fopen(gff_fname, 'w+');
  if CFG.VERBOSE>1, disp(msg); end
end
...
for g = 1:length(genes),
  if CFG.VERBOSE>0, fprintf(1, 'writing gene %i...\r', g); end
  gene = genes(g);
  type  = 'gene';
  score = '.';
  phase = '.';
