...
if [ "$1" == 'big' ];
then
  echo Note: Running this script takes about 3 hours \(on a single CPU\).
...
INTRON_DISTS_FN="dummy_id_in"
LEARN_PROFILES="0"
NUM_ITER="1"
PROFILES_FN_OUT="dummy_pr_out"
INTRON_DISTS_FN_OUT="dummy_id_out"
...
../bin/rquant ${GENES_RES_DIR} ${BAM_INPUT} ${RQUANT_RES_DIR}/${EXP}_rquant.gff3 ${RQUANT_RES_DIR}/ ${LOAD_PROFILES} ${PROFILES_FN} ${INTRON_DISTS_FN} ${LEARN_PROFILES} ${NUM_ITER} ${PROFILES_FN_OUT} ${INTRON_DISTS_FN_OUT} > ${RESULTDIR}/elegans-rquant.log
